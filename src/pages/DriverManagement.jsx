    ...
    return (
        <div className="flex flex-col h-full space-y-4 sm:space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-xl sm:text-2xl font-bold text-fg">Driver Management</h1>
            ...
            <div className="flex flex-col flex-1 p-4 sm:p-6 bg-surface border rounded-lg border-line ">
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6 gap-4">
                    <div className="relative w-full md:max-w-sm">
                        <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                            <svg className="w-5 h-5 text-fg-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div className="flex flex-row items-center space-x-2 sm:space-x-4 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
                        <div className="flex items-center space-x-2 flex-shrink-0">
                            <span className="hidden text-sm font-medium sm:inline text-fg-3">Vehicle:</span>
                            <select
                                value={filterVehicleType}
                                onChange={(e) => setFilterVehicleType(e.target.value)}
                                className="block w-28 sm:w-32 py-2 pl-3 pr-8 sm:pr-10 text-xs sm:text-sm border border-line rounded-lg focus:outline-none focus:ring-primary-500 focus:border-primary-500 bg-surface"
                            >
                        ...
                        <div className="flex items-center space-x-2 flex-shrink-0">
                            <span className="hidden text-sm font-medium sm:inline text-fg-3">Status:</span>
                            <select
                                value={filterStatus}
                ...
                <div className="flex-1 min-h-0 overflow-y-auto sm:overflow-x-auto sm:border sm:rounded-lg sm:border-line">

                    {/* Desktop Table View */}
                    <table className="min-w-full divide-y divide-line hidden md:table">
                        <thead className="bg-raised sticky top-0">
                            <tr>
                                <th scope="col" className="px-6 py-3 text-xs font-semibold tracking-wider text-left text-fg-3 uppercase">
                                    Driver Info
                                </th>
                                <th scope="col" className="px-6 py-3 text-xs font-semibold tracking-wider text-left text-fg-3 uppercase">
                                    Contact
                                ...
                                <th scope="col" className="px-6 py-3 text-xs font-semibold tracking-wider text-center text-fg-3 uppercase">
                                    Action
                                </th>
                            </tr>
                        </thead>
                        <tbody className="bg-surface divide-y divide-line">
                            {loading ? (
                                <tr>
                                    <td colSpan="6" className="px-6 py-12 text-center text-sm text-fg-3">
                                ...
                                filteredDrivers.map((driver) => (
                                    ...
                                    >
                                            ...
                                            <div className="flex items-center">
                                                <div className="relative">
                                                    ...
                                                    {driver.hasSubscription && (
                                                        <div className="absolute -top-1 -right-1 w-3 h-3 bg-warn border-2 border-line rounded-full" title="Active Subscription"></div>
                                                    )}
                                                </div>
                                                <div className="ml-4 flex items-center">
                                                    <div>
                                                        <div className="text-sm font-medium text-fg focus:outline-none flex items-center">
                                                            {driver.name || 'Unknown'}
                                                            {!driver.isApproved && driver.createdAt && (
                                                                (() => {
                                                                    ...
                                                                    <span className="ml-2 inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold bg-warn-soft text-warn uppercase tracking-wider">
                                                                        New
                                                                    </span>
                                                                )}
                                                        </div>
                                                        <div className="text-sm text-fg-3">ID: {driver.id}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <div className="text-sm text-fg">{driver.phone || 'N/A'}</div>
                                            <div className="text-sm text-fg-3">{driver.email || 'N/A'}</div>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className="px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-md bg-raised text-fg">
                                                {driver.vehicleType || 'Unassigned'}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-fg-3">
                                            {(() => {
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className={`px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${driver.isApproved ? 'bg-ok-soft text-ok' : 'bg-danger-soft text-danger'
                                                }`}>
                    ...
                    <div className="md:hidden flex flex-col space-y-4">
                        {loading ? (
                            <div className="py-12 text-center text-sm text-fg-3 bg-raised rounded-lg border border-line border-dashed">
                                <div className="flex flex-col items-center justify-center">
                                    <div className="w-8 h-8 mb-4 border-4 border-primary-200 rounded-full border-t-primary-600 animate-spin"></div>
                                    <p>Loading driver data...</p>
                                </div>
                            ...
                            filteredDrivers.map((driver) => (
                                <div
                                    key={driver.id}
                                    onClick={() => handleRowClick(driver)}
                                    className="bg-surface border text-left border-line rounded-lg p-4 transition-shadow cursor-pointer relative"
                                >
                                    <div className="flex items-center justify-between mb-3">
                                        <div className="flex items-center space-x-3">
                                            <div className="relative">
                                                ...
                                                {driver.hasSubscription && (
                                                    <div className="absolute -top-1 -right-1 w-3.5 h-3.5 bg-warn border-2 border-line rounded-full"></div>
                                                )}
                                            </div>
                                            <div>
                                                <div className="font-bold text-fg flex items-center">
                                                    {driver.name || 'Unknown'}
                                                    {!driver.isApproved && driver.createdAt && (
                                                        (() => {
                                                            const createdAtDate = driver.createdAt?.toDate ? driver.createdAt.toDate() : new Date(driver.createdAt);
                                                            ...
                                                            <span className="ml-2 px-1.5 py-0.5 rounded text-[10px] font-bold bg-warn-soft text-warn uppercase">New</span>
                                                        )}
                                                </div>
                                                <div className="text-xs text-fg-3 mt-0.5 tracking-tight">ID: {driver.id?.substring(0, 10)}...</div>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-sm mb-4 bg-raised p-3 rounded-lg border border-line/60">
                                        <div>
                                            <div className="text-[10px] font-semibold text-fg-3 uppercase tracking-wider mb-0.5">Contact</div>
                                            <div className="text-fg font-medium truncate">{driver.phone || 'N/A'}</div>
                                        </div>
                                        <div>
                                            <div className="text-[10px] font-semibold text-fg-3 uppercase tracking-wider mb-0.5">Vehicle</div>
                                            <span className="px-2 py-0.5 inline-flex text-xs font-semibold rounded bg-surface border border-line text-fg-2">
                                                {driver.vehicleType || 'N/A'}
                                            </span>
                                        </div>
                                        <div className="col-span-2">
                                            <div className="text-[10px] font-semibold text-fg-3 uppercase tracking-wider mb-0.5">Joined</div>
                                    ...
                                    <div className="flex items-center justify-between pt-1 border-t border-line/60">
                                        <span className={`px-2.5 py-1 text-[11px] font-bold rounded-full ${driver.isApproved ? 'bg-ok-soft text-ok' : 'bg-danger-soft text-danger'}`}>
                                            {driver.isApproved ? 'Approved' : 'Not Approved'}
                                        </span>
                                        {driver.status === 'Blocked' && !driver.isApproved ? (
                                            ...
                                            <button onClick={(e) => { e.stopPropagation(); handleToggleClick(e, driver); }} className={`px-4 py-1.5 text-brand-fg text-xs font-semibold rounded-lg ${driver.isApproved ? 'bg-danger-soft text-danger border border-danger/30' : 'bg-primary-600'}`}>
                                                {driver.isApproved ? 'De-approve' : 'Approve'}
                                            </button>
                                        )}
                                    </div>
                ...
                {/* Pagination Dummy UI */}
                <div className="flex items-center justify-between px-4 py-3 mt-4 border-t border-line sm:px-6">
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 overflow-y-auto">
                    <div className="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                        <div className="fixed inset-0 z-0 transition-opacity bg-canvas/80 backdrop-blur-sm" onClick={() => !actionLoading && setIsModalOpen(false)}></div>
                        <span className="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
                        <div className="relative z-10 inline-block px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-surface rounded-lg shadow-2xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
                            <div className="sm:flex sm:items-start">
                                <div className={`flex items-center justify-center flex-shrink-0 w-12 h-12 mx-auto rounded-full sm:mx-0 sm:h-10 sm:w-10 ${selectedDriver?.isApproved ? 'bg-danger-soft' : selectedDriver?.status === 'Blocked' ? 'bg-raised' : 'bg-primary-100'}`}>
                                    {selectedDriver?.isApproved ? (
                                        <svg className="w-6 h-6 text-danger" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                                        </svg>
                                    ) : selectedDriver?.status === 'Blocked' ? (
                                        <svg className="w-6 h-6 text-fg-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                    <h3 className="text-lg font-medium leading-6 text-fg" id="modal-title">
                            ...
                            <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                                <button
                                    ...
                                    className={`inline-flex justify-center w-full px-4 py-2 text-base font-medium text-brand-fg border border-transparent rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 sm:ml-3 sm:w-auto sm:text-sm transition-colors ${selectedDriver?.isApproved
                                        ? 'bg-danger hover:bg-danger focus:ring-danger/40 disabled:bg-danger'
                                ...
                                <button
                                    type="button"
                                    disabled={actionLoading}
                                    className="inline-flex justify-center w-full px-4 py-2 mt-3 text-base font-medium bg-surface border rounded-md border-line text-fg-2 hover:bg-raised focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 sm:mt-0 sm:w-auto sm:text-sm"
